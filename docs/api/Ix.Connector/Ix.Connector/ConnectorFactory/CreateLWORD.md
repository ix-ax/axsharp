# ConnectorFactory.CreateLWORD method (1 of 2)

Creates UInt64 tag of a value type DWORD;

```csharp
public abstract OnlinerLWord CreateLWORD(ITwinObject parent, string readableTail, string symbolTail)
```

| parameter | description |
| --- | --- |
| parent | Parent object of [`ITwinObject`](../ITwinObject.md) type. |
| readableTail | Human readable tail of this value tag. |
| symbolTail | Symbol tail of this value tag. |

## Return Value

New instance of [`OnlinerLWord`](../../Ix.Connector.ValueTypes/OnlinerLWord.md)

## See Also

* class [OnlinerLWord](../../Ix.Connector.ValueTypes/OnlinerLWord.md)
* interface [ITwinObject](../ITwinObject.md)
* class [ConnectorFactory](../ConnectorFactory.md)
* namespace [Ix.Connector](../../Ix.Connector.md)

---

# ConnectorFactory.CreateLWORD method (2 of 2)

Creates UInt64 empty tag of a value type LWORD;

```csharp
public static OnlinerLWord CreateLWORD()
```

## See Also

* class [OnlinerLWord](../../Ix.Connector.ValueTypes/OnlinerLWord.md)
* class [ConnectorFactory](../ConnectorFactory.md)
* namespace [Ix.Connector](../../Ix.Connector.md)

<!-- DO NOT EDIT: generated by xmldocmd for Ix.Connector.dll -->
